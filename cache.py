...
# Збереження останнього ID для уникнення дублювання
last_processed_id = {}
...
def resend_cached_data():
    ...
    for row in rows:
        try:
            ...
            conn_cache.commit()
            print(f"Resent cached data to Influx: {row[2]}={row[3]} at {row[5]}")
        except Exception as e:
...
while True:
    for table in tables:
        ...
        if last_record:
            ...
            if table in last_processed_id and last_processed_id[table] == record_id:
                print(f"Skipping duplicate record {record_id} for table {table}")
                continue

            last_processed_id[table] = record_id
            radio_state = last_record[12]
            if radio_state == 0:
                current_amounts = last_record[0:4]
